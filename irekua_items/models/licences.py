...
class Licence(IrekuaModelBaseUser):
    ...
    The licence type (:model:`irekua_database.LicenceType`) contains all
    details about permissions and also holds a template document to be
    filled whenever creating a new licence of such type.

    A single licence can only apply to items (:model:`irekua_database.Item`)
    within the same collection (:model:`irekua_database.Collection`),
    therefore a reference to the collection to which the licence belongs
    is stored. This facilitates the process of attaching a licence to uploaded
    items, since the number of licences belonging to a single collection is
    recommended to be small.
