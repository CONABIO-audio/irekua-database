...
class DeploymentType(IrekuaModelBase, MetadataSchemaMixin):
    ...
    device_type = models.ForeignKey(
        ...
        help_text=_(
            "Type of device that can be used in the deployment of the given type"
        ),
        null=False,
        blank=False,
    )
    ...
    def validate_device_type(self, device_type):
        if self.device_type != device_type:
            ...
            params = dict(
                deployment_type=self, this=self.device_type, other=device_type
            )
    ...
    def validate_item_type(self, item_type):
        ...
        if not self.item_types.filter(pk=item_type.pk).exists():
            ...
            params = dict(deployment_type=str(self), item_type=str(item_type))
            raise ValidationError(msg % params)
