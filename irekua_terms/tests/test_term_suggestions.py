...
class TermSuggestionTestCase(TestCase):
    ...
    def test_repr(self, value):
        ...
        self.assertEqual(str(suggestion), f"categorical1: {value}")

    @given(valid_string())
    def test_clean_non_categorical_types(self, value):
        with self.assertRaises(ValidationError):
            TermSuggestion(
                term_type=self.boolean, value=value, metadata=None
            ).clean()

        with self.assertRaises(ValidationError):
            TermSuggestion(
                term_type=self.integer, value=value, metadata=None
            ).clean()

        with self.assertRaises(ValidationError):
            TermSuggestion(
                term_type=self.numeric, value=value, metadata=None
            ).clean()
    ...
    def test_clean_valid_metadata(self, value, metadata):
        TermSuggestion(
            term_type=self.categorical, value=value, metadata=metadata
        ).clean()
