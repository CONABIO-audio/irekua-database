...
class OrganismCapture(IrekuaModelBaseUser):
    ...
    collection_metadata = models.JSONField(
        ...
        help_text=_(
            "Additional metadata associated to organism capture in collection"
        ),
        blank=True,
        null=True,
    )
    ...
    def clean(self):
        ...
        organism_capture_type_config = self.clean_organism_capture_type(
            organism_config
        )

        # Check that additional collection metadata is valid for organism
        # capture type
    ...
    def clean_compatible_organism_and_capture_type(self):
        ...
        if (
            ...
            != self.organism_capture_type.organism_type
        ):
            msg = _(
                "Captures of type %(capture_type)s cannot be used on "
                "organisms of type %(organism_type)s"
            )
            # pylint: disable=no-member
            params = dict(
    ...
    def clean_organism_capture_type(self, organism_config):
        try:
            return organism_config.get_organism_capture_type(
                self.organism_capture_type
            )
    ...
    def clean_collection_metadata(self, organism_capture_type_config):
        try:
            organism_capture_type_config.validate_metadata(
